...
class CraftingRegister
{
    ...
    private static ?PrefixedLogger $logger = null;

    const SHAPED_TYPE = 'shaped';
    const SHAPELESS_TYPE = 'shapeless';

    private static function getLogger(): PrefixedLogger
    {
        return self::$logger ??= new PrefixedLogger(GlobalLogger::get(), 'CraftingRegister');
    }
    ...
    private static function load_recipes_by_folder(string $folder): void
    {
        try {
                ...
                switch ($recipe_data->getRecipeType()) {
                    case RecipeData::SHAPED_TYPE:
                        try {
                            ...
                            self::getLogger()->error("Error processing data from " . $recipe_data->getIdentifier() . ": " . $th->getMessage());
                        }
                        break;
            ...
            self::getLogger()->error($th->getMessage());
        }
    }
