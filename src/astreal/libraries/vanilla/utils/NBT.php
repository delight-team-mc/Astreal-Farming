...
namespace astreal\libraries\vanilla\utils;
...
class NBT{
	...
	private static function getArrayTag(array $array): Tag {
		if(array_keys($array) === range(0, count($array) - 1))return new ListTag(array_map(fn($value) => self::getTagType($value), $array));
		$tag = CompoundTag::create();
		foreach($array as $key => $value)$tag->setTag($key, self::getTagType($value));
		return $tag;
	}
}